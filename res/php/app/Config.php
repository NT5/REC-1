...
class Config extends \REC1\Factory\BaseComponents {
     ...
     */
    public function __construct($page_title = NULL, $first_run = TRUE, \REC1\Factory\BaseComponents $BaseComponents = NULL) {
        if (!$BaseComponents) {
        ...
        $this->setLog(\REC1\Util\Logger\Areas::CORE_CONFIG, "Nueva instancia de configuración de pagina creada");
    }
    ...
    public function saveToIni($ini = 'config.ini') {
        ...
        $this->setLog(\REC1\Util\Logger\Areas::CORE_CONFIG, "Intentando guardar configuración en el archivo $ini...");
        ...
        foreach ($data as $index => $value) {
            if (\REC1\Util\Files::set_ini_file($ini, $ini_area, $index, $value)) {
                $this->setLog(\REC1\Util\Logger\Areas::CORE_CONFIG, "La variable %s fue guardada correctamente con el valor: %s", $index, $value);
                continue;
            } else {
                $this->setLog(\REC1\Util\Logger\Areas::CORE_CONFIG_ERROR, "No se pudo guardar el archivo de configuración; operacion abortada");
                $this->addWarning(new \REC1\Warning(\REC1\Warning\Warnings::CANT_SAVE_PAGE_CONFIG_FILE));

                return FALSE;
            }
        }
        $this->setLog(\REC1\Util\Logger\Areas::CORE_CONFIG, "El archivo $ini fue guardado correctamente");
        return TRUE;
    }
     ...
     * @param \REC1\Factory\BaseComponents $BaseComponents
     * @return \REC1\Config Regresa instancia de configuracion creada
     */
    public static function fromIniFile($inifile = NULL, \REC1\Factory\BaseComponents $BaseComponents = NULL) {
        ...
        $BaseComponents->setLog(\REC1\Util\Logger\Areas::CORE_CONFIG, "Intentando crear configuración con $inifile...");

        $ini = \REC1\Util\Files::load_ini_file($inifile);

        if ($ini) {

            $BaseComponents->setLog(\REC1\Util\Logger\Areas::CORE_CONFIG, "Comprobando estructura de $inifile...");
            ...
            if (\REC1\Util\Functions::checkArray([$ini_area], $ini) && \REC1\Util\Functions::checkArray($valid_structure, $ini[$ini_area])) {
                ...
                $BaseComponents->setLog(\REC1\Util\Logger\Areas::CORE_CONFIG, "Instancia de configuración creada correctamente con $inifile");

                return $instance;
            } else {
                $BaseComponents->setLog(\REC1\Util\Logger\Areas::CORE_CONFIG_ERROR, "El archivo $inifile tiene una estructura invalida");

                $BaseComponents->addWarning(new \REC1\Warning(\REC1\Warning\Warnings::PAGE_CONFIGURATION_INVALID_FORMAT));
                $BaseComponents->addWarning(new \REC1\Warning(\REC1\Warning\Warnings::DEFAULT_PAGE_CONFIGURATION));

                return new self(NULL, NULL, $BaseComponents);
            }
        } else {
            $BaseComponents->setLog(\REC1\Util\Logger\Areas::CORE_CONFIG_ERROR, "El archivo $inifile no pudo ser cargado");

            $BaseComponents->addWarning(new \REC1\Warning(\REC1\Warning\Warnings::CANT_LOAD_PAGE_CONFIGURATION_FILE));
            $BaseComponents->addWarning(new \REC1\Warning(\REC1\Warning\Warnings::DEFAULT_PAGE_CONFIGURATION));
